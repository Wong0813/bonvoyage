...
class AdminPackageFormDialog extends StatefulWidget {
  final TravelPackageModel? pkg;
  final List<AgentProfileModel> agentProfiles;
  final VoidCallback onSaved;

  const AdminPackageFormDialog({
    super.key,
    this.pkg,
  ...
  @override
  State<AdminPackageFormDialog> createState() => _AdminPackageFormDialogState();
}

class _AdminPackageFormDialogState extends State<AdminPackageFormDialog> {
  ...
  void initState() {
    ...
    _selectedAgentId = widget.pkg?.agentId ?? (widget.agentProfiles.isNotEmpty ? widget.agentProfiles.first.id : null);
  ...
  Future<void> _submit() async {
    ...
    final maxPeople = int.tryParse(_maxCtrl.text) ?? 10;
    final promoPrice = double.tryParse(_promoCtrl.text);

    if (_selectedAgentId == null) {
      showAppSnackBar(context, 'Please select a travel agent.', isError: true);
      return;
    }
    if (_destCtrl.text.trim().isEmpty || _descCtrl.text.trim().isEmpty || price <= 0) {
    ...
    try {
      if (widget.pkg == null) {
        await TravelPackageService.instance.createPackage(
          agentProfileId: _selectedAgentId!,
          ...
          promoEnd: promoPrice != null ? _travelDate : null,
          scheduleFilePath: _schedulePath,
          images: _imagePaths,
          category: _category,
        );
        if (mounted) showAppSnackBar(context, 'Package created successfully!');
      } else {
        await TravelPackageService.instance.adminUpdatePackage(
          id: widget.pkg!.id,
          destination: _destCtrl.text.trim(),
          description: _descCtrl.text.trim(),
          attractions: _attCtrl.text.trim(),
        ...
        if (mounted) showAppSnackBar(context, 'Package updated successfully!');
      }
      widget.onSaved();
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      title: Text(
        widget.pkg == null ? 'Create Travel Package' : 'Edit Package Details',
      ...
      content: SingleChildScrollView(
        child: SizedBox(
          ...
          child: Column(
            ...
            children: [
              const Text('* indicates a required field', style: TextStyle(color: Colors.white38, fontSize: 11, fontStyle: FontStyle.italic)),
              const SizedBox(height: 12),
              if (widget.pkg == null) ...[
                DropdownButtonFormField<int>(
                  value: _selectedAgentId,
                  dropdownColor: const Color(0xFF16162A),
                  decoration: AppTheme.input('Hosting Agent *'),
                  items: widget.agentProfiles.map((a) {
                    return DropdownMenuItem<int>(
                      value: a.id,
                      child: Text(a.companyName, style: const TextStyle(color: Colors.white, fontSize: 13)),
                    );
                  }).toList(),
                ...
                DropdownButtonFormField<int>(
                  value: _selectedAgentId,
                  dropdownColor: const Color(0xFF16162A),
                  decoration: AppTheme.input('Hosting Agent (Reassign)'),
                  items: widget.agentProfiles.map((a) {
                    return DropdownMenuItem<int>(
                      value: a.id,
                      child: Text(a.companyName, style: const TextStyle(color: Colors.white, fontSize: 13)),
                ...
                DropdownButtonFormField<String>(
                  value: _status,
                  dropdownColor: const Color(0xFF16162A),
                  decoration: AppTheme.input('Status'),
                  items: const [
              ...
              TextField(controller: _destCtrl, decoration: AppTheme.input('Destination *'), style: const TextStyle(color: Colors.white)),
              const SizedBox(height: 12),
              TextField(controller: _descCtrl, decoration: AppTheme.input('Package Description *'), maxLines: 3, style: const TextStyle(color: Colors.white)),
              const SizedBox(height: 12),
              TextField(controller: _attCtrl, decoration: AppTheme.input('Attractions (one per line)'), maxLines: 3, style: const TextStyle(color: Colors.white)),
              const SizedBox(height: 12),
              DropdownButtonFormField<String>(
                value: _tripType,
                dropdownColor: const Color(0xFF16162A),
              ...
              const SizedBox(height: 12),
              TextField(controller: _maxCtrl, decoration: AppTheme.input('Max Group Size *'), keyboardType: TextInputType.number, style: const TextStyle(color: Colors.white)),
              const SizedBox(height: 12),
              TextField(controller: _priceCtrl, decoration: AppTheme.input('Price Per Person (RM) *'), keyboardType: TextInputType.number, style: const TextStyle(color: Colors.white)),
              const SizedBox(height: 12),
              TextField(controller: _promoCtrl, decoration: AppTheme.input('Promo Price (RM, optional)'), keyboardType: TextInputType.number, style: const TextStyle(color: Colors.white)),
              const SizedBox(height: 12),
              ListTile(
              ...
              ElevatedButton.icon(
                onPressed: () async {
                  final r = await FilePicker.platform.pickFiles(type: FileType.custom, allowedExtensions: ['pdf', 'doc', 'docx', 'txt']);
              ...
              if (_schedulePath != null)
                Padding(
                  ...
                  child: Text(
                    'Attached: ${_schedulePath!.split('/').last.split('\\').last}',
      ...
      actions: [
        ...
        ElevatedButton(
          ...
          child: _saving
              ...
              : Text(widget.pkg == null ? 'Create Package' : 'Save Details', style: const TextStyle(fontWeight: FontWeight.bold)),
        ),
      ],
    );
  }
